		...
		return new Promise((resolve, reject) => {
			if (frappe.query_reports[this.report_name]) {
				...
				frappe.xcall('frappe.desk.query_report.get_script', {
					...
					frappe.dom.eval(res.script || '');
					// res.script can have ajax calls
					frappe.after_ajax(() => {
						this.report_settings = this.get_local_report_settings();
						this.report_settings.html_format = res.html_format;
						this.report_settings.execution_time = res.execution_time || 0;
